...
The top `Live Market` section polls Binance public REST endpoints every 15 seconds for ETH/USDT 24h ticker data and configurable recent klines. It shows last price, 24h percentage change, quote volume, high/low, last refresh time, a compact market snapshot strip, and a range-adjustable chart. The chart supports line/range/candle views and optional decision overlays for 24h high/low, entry, take-profit, trailing-stop, and invalidation levels. If live polling fails, the panel keeps the previous values when available and displays a stale or failed badge with the source error.
